...
miss_funs =     [induce_mar]                      
imp_funs =      [ffill_median]
miss_types =    ['single', 'patient', 'both'] 
miss_weights =  ['equal', 'squared', 'exponential']
cond_vars =      ['age']

combinations = list(product(datasets, miss_rates, miss_funs, imp_funs, miss_types, miss_weights, cond_vars))

#  induce_mar(df, cond_var, miss_weights, miss_rate, miss_type, ignore_cols=[]):

def calculate_errors(options, run):
    result = []
    for dfkey, missing_rate, missfun, impfun, miss_type, miss_weight, cond_var in options:
        df = datasets[dfkey]
        df2 = missfun(df, cond_var, miss_weight, missing_rate, miss_type, no_miss_cols)
