...
miss_funs =     [induce_mcar]                      
imp_funs =      [ffill_median]
miss_types =    ['single', 'patient', 'both'] 

combinations = list(product(datasets, miss_rates, miss_funs, imp_funs, miss_types))


def calculate_errors(options, run):
    result = []
    for dfkey, missing_rate, missfun, impfun, miss_type in options:
        
        df = datasets[dfkey]
        df2 = missfun(df, missing_rate, miss_type, no_miss_cols)
        ...
        result.append([run, miss_type,dfkey, missing_rate, missrates_exact, mse, mae, rmse])
...
if __name__ == '__main__':
    ...
    res_cols = ['run','missing_pattern', 'dataset', 'missing_rate', 'missing_rate_exact', 'mse', 'mae', 'rmse']
